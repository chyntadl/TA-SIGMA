...
<body>
    <div class="flex items-center justify-center h-2/4">
        <div class="px-1 py-6 mt-4 text-left shadow-lg rounded-lg bg-green-200 w-2/4 pr-8 pl-8">
            <h3 class="text-2xl font-bold text-center text-sky-900">Data Peserta Magang</h3>
            <form class="mt-4">
                <div class="flex flex-row mb-3">
                    <input type="hidden" id="id">
                    <div class="basis-1/2 mr-4">
                        <div>NPM :</div>
                        <input type="number" disabled  class="form-input px-2 py-2 rounded-lg w-full mb-2" id="npm">
                ...
                <div class="text-center">
                    ...
                    <button type="button" class="bg-green-500 pl-8 pr-7 pt-2 pb-2 text-white inline-block m-3 rounded shadow hover:bg-green-700" onclick="tambah()">UPDATE</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <script>
        ...
        fetch('{{ $server }}/api/magang/' + localStorage.getItem('id'), {
                method: 'GET',
                headers: {
            ...
            .then(data => {
                document.getElementById('id').value = data.magang.id;
                document.getElementById('npm').value = data.magang.npm;
                document.getElementById('nama_lengkap').value = data.magang.nama_lengkap;
                document.getElementById('jurusan').value = data.magang.jurusan;
                document.getElementById('perguruan').value = data.magang.perguruan_tinggi;
                document.getElementById('telepon').value = data.magang.nomor_handphone;
                document.getElementById('email').value = data.magang.email;
                document.getElementById('password').value = data.magang.password;
                document.getElementById('devisi').value = data.magang.devisi;
            })
